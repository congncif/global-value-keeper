#
# Be sure to run `pod lib lint GlobalValueKeeper.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see https://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'GlobalValueKeeper'
  s.version          = '0.4.0'
  s.swift_versions   = ["5.0", "5.1", "5.2", "5.3"]
  s.summary          = 'A testable approach for global state.'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
* Prevent to access directly to a singleton which will be hard to test.
* Avoid to pass a short-lifetime states across some objects that can make your logic more complex.
                       DESC

  s.homepage         = 'https://github.com/congncif/global-value-keeper'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'CONG NGUYEN CHI' => 'congnc.if@gmail.com' }
  s.source           = { :git => 'https://github.com/congncif/global-value-keeper.git', :tag => s.version.to_s }
   s.social_media_url = 'https://twitter.com/congncif'

  s.ios.deployment_target = '8.0'

  s.source_files = 'GlobalValueKeeper/**/*.swift'
  
  # s.resource_bundles = {
  #   'GlobalValueKeeper' => ['GlobalValueKeeper/Assets/*.png']
  # }

  # s.public_header_files = 'Pod/Classes/**/*.h'
  # s.frameworks = 'UIKit', 'MapKit'
  # s.dependency 'AFNetworking', '~> 2.3'
end
